...
Разворачиваем стенд, где openVPN используется для запуска трёх серверов, на обной виртуальной машине srv01:
...
-  туннель создаётся между srv01 - "хост машина"
    ...
    Этот файл необходимо поместить в папку /etc/openvpn/client, и выполнить:

        systemctl start openvpn-client@192.168.128.1

192.168.160.2:
- запускается на порту 1195 для организации туннеля типа tun
- туннель создаётся между srv01 - cln01
192.168.192.2:
- запускается на 1196 порту, для организации туннеля типа tap
- туннель создаётся между srv01 - cln02. Для теста соединения используется виртуальная машина cln04, эмулирующая наличие сети за srv01.  

  на cln04 запустим сервер iperf3 для теста соединения:

      iperf3 -s

  на cln02 запустим тест канала:

      iperf3 -c 192.168.224.7 -i 5 -t 60 -b50M -u



Вариант создания tap туннеля:

      ip tunnel add tap1 mode gre remote 192.168.192.2 local 192.168.192.3  dev tap1- туннель на cln02

      ip tunnel add tap1 mode gre remote 192.168.192.3 local 192.168.192.2 dev eth3 - тунель на srv01


**Создание правил для пропуска трафика через тунель L2 уровня, в openVPN устройство типа "tap":**
Для работы создаём бридж интерфейс и на него навешиваем ip (192.168.224.2/28), но правила вешаем на физический интерфейс, входящий в состав моста. "tap" интерфейс тоже входит в состав моста  

      ebtables -t nat -A POSTROUTING -o eth4 -j snat --to-source 08:00:27:95:bf:0e --snat-arp - натировать исходящие arp запросы в mac физичекского интерфейса "eth4", при ответе пакеты идут unicast  
      ebtables -t nat -A PREROUTING -p ARP -i eth4 --arp-ip-dst 192.168.224.3 -j dnat --to-dst 08:00:27:ca:86:09 - натировать входящие запросы на физическом интерфейсе, для возврата клиенту через туннель.
...
**Файлы конфигурации сервера и клиента нужно положить в папку /etc/openvpv/server и /etc/openvpv/client соответственно**
